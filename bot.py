        ...
        def _home(self):
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "*Each one has its own editor to fill in all the details: bios, lore, ages, species, playlists, and more.*\n\n"
                    "🖼 *No personal character art yet? No worries! This bot is for fun, so feel free to use art online as a placeholder with `/char build`~*\n\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                    "📚 **Stories** — Add, browse, and manage fics\n"
                    "🧬 **Characters** — Create, build, and browse favorites\n"
                    "🎨 **Fanart** — Upload and browse YOUR/OTHER USER'S owned artwork\n"
                    "🌸 **Author** — Profiles, badges, and reader journey\n\n"
                    "🃏 **Character Trading Cards** — Use `/ctc help` for the full CTC game!\n\n"
...
pvp_group      = app_commands.Group(name="pvp",       description="Elemental PVP card battles",  guild_ids=[GUILD_ID])
...
# =====================================================
# /char setmc — mark up to 3 characters as Main Characters per story
# =====================================================

class _SetMCModal(ui.Modal, title="Set Main Characters"):
    def __init__(self, story_id: int, story_title: str, defaults: list):
        super().__init__()
...
async def char_setmc(interaction: discord.Interaction, story: str):
    ...
    try:
        ...
        await interaction.response.send_message("❌ Invalid story selection.", ephemeral=True, delete_after=4)
        return
    ...
    story_title = story_row.get("title", "Unknown Story")

    # Pre-populate with current MCs for this story
    current_mcs = [
        c["name"] for c in get_mc_characters_for_user(uid)
...
bot.tree.add_command(pvp_group)
...
from features.pvp.pvp_commands import register_pvp_commands
register_pvp_commands(pvp_group, GUILD_ID)
